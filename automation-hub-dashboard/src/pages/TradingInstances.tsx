...
const runtimeHealth = (instance: Instance) => instance.ui_status === "ERROR" ? "Error — entries blocked" : instance.ui_status === "BLOCKED" ? `${instance.strategy_health?.status === "Unhealthy" ? "Unhealthy" : "Blocked"} — ${instance.last_blocker ?? "entries disabled"}` : instance.ui_status === "RUNNING_UNARMED" ? "Running unarmed" : instance.strategy_health?.status === "Unhealthy" ? "Unhealthy — entries blocked" : "Running armed";
    ...
    <Card className="instance-active-card" title="Active Trading Instances" subtitle="live runtime is separate from historical performance" right={<Badge text={`${active.filter((row) => row.state === "running").length} running · ${active.length} active`} tone={active.length ? "green" : "default"} />}>
      {view === "table" ? <div className="tablewrap"><table className="data-table"><thead><tr><th>Pair</th><th>Strategy</th><th>Version</th><th>TF</th><th>State</th><th>Capital</th><th>Risk</th><th>P&L</th><th>Trades</th><th>WR</th><th>PF</th><th>Market data</th><th>Position</th><th>Health</th><th></th></tr></thead><tbody>
        ...
        {active.map((row) => <article key={row.id} className="instance-worker-row">
          <section className="instance-worker-column instance-identity"><div className="dim" style={{ fontSize: 10 }}>FORWARD_PAPER · BINANCE USD-M</div><Badge text={row.ui_status ?? "BLOCKED"} tone={statusTone(row.ui_status) as any} /><h3>{row.symbol}</h3><div>{row.strategy_label}</div><div className="dim">{row.strategy_key} · {row.strategy_version} · live market data · simulated orders · no exchange routing</div>{row.strategy_identity?.matches === false && <div className="instance-risk-notice red">Configured {row.strategy_identity.configured_label} differs from worker {row.strategy_identity.worker_label ?? "unknown"}.</div>}{(row.state === "error" || row.state === "degraded" || actionErrors[row.id]) && <div className="instance-risk-notice red" style={{ marginTop: 8 }}><b>{errorFor(row) || "No error reason recorded"}</b><br /><span className="dim">Recommended: {recoveryGuidance(errorFor(row))}</span></div>}<RebootProgress reboot={row.reboot} /><div className="instance-worker-actions"><InstanceActions instance={row} action={action} remove={remove} actionBusy={actionBusy} locked={controlsUnavailable} deleting={deletingId === row.id} /></div><button className="btn btn-link" onClick={() => setSelected(row.id)}>Open details</button></section>
          ...
          <section className="instance-worker-column"><Detail label="Worker / heartbeat" value={`${row.engine?.worker_task_identifier ?? "—"} / ${timestamp(row.engine?.last_heartbeat)}`} /><Detail label="Signals / accepted / rejected" value={`${row.worker_counts?.signals ?? 0} / ${row.worker_counts?.accepted ?? 0} / ${row.worker_counts?.rejections ?? 0}`} /><Detail label="Rejections learning / risk / corr / dedup" value={`${row.engine?.rejection_counts?.learning ?? 0} / ${row.engine?.rejection_counts?.risk ?? row.engine?.rejection_counts?.risk_guard ?? 0} / ${row.engine?.rejection_counts?.correlation ?? 0} / ${row.engine?.rejection_counts?.dedup ?? 0}`} /><Detail label="Last decision" value={row.last_decision ? `${row.last_decision.final_state ?? "QUALIFIED"} · ${row.last_decision.blocker || row.last_decision.reason}` : row.last_blocker ?? "No decision"} /><Detail label="Open positions / orders" value={`${row.engine?.open_positions ?? "—"} / ${row.execution?.pending_orders ?? "—"}`} /><Detail label="Open risk / Unrealized" value={`${money(row.risk?.open_risk_amount)} / ${signedMoney(row.execution?.unrealized_pnl)}`} /></section>
        </article>)}
