...
type Instance = { id: string; symbol: string; strategy_key: string; strategy_label: string; strategy_version: string; timeframe: string; risk_per_trade_pct: number; capital_allocation: number; sizing_mode?: string; fixed_position_size?: number; entry_mode?: string; fill_model?: string; execution_mode?: string; market_data_mode?: string; mode: string; state: string; created_at: string; started_at?: string | null; stopped_at?: string | null; last_error?: string; metrics: Metric; performance?: Metric; execution?: Metric; risk?: Metric; engine?: Metric | null; market_data?: MarketData; current_position?: Metric | null; strategy_health?: Metric | null; last_decision?: Metric | null };
type InstancesResponse = { instances: Instance[]; max_active_slots: number; active_slots: number; max_global_risk_pct: number; max_global_risk_amount: number; current_global_risk_amount: number; paper_account_capital?: number; total_allocated_capital?: number; total_current_equity?: number; available_paper_capital?: number; today_pnl?: number; today_trades?: number; total_open_positions?: number; global_risk_status?: string; market_data_status?: string; global_status?: string; instance_counts?: Record<string, number> };
    ...
    <div className="instances-control-grid">
      <Card className="instance-control-card" title="Create Trading Instance" subtitle="1 pair · 2 strategy · 3 version · 4 timeframe · 5 capital · 6 risk · 7 sizing · 8 entry · 9 review · 10 start">
        <div className="form-grid-2">
          ...
          <Field label="Fixed quantity" hint="used only for Fixed Quantity"><input value={form.fixed_position_size} disabled={form.sizing_mode !== "fixed"} onChange={(e) => setForm({ ...form, fixed_position_size: e.target.value })} inputMode="decimal" /></Field>
    ...
    <Card className="instance-active-card" title="Active Trading Instances" subtitle="each card is a separate worker, ledger scope, strategy state, and paper execution account" right={<Badge text={`${active.length} running`} tone={active.length ? "green" : "default"} />}>
      {view === "table" ? <div className="tablewrap"><table className="data-table"><thead><tr><th>Pair</th><th>Strategy</th><th>Version</th><th>TF</th><th>State</th><th>Capital</th><th>Risk</th><th>P&L</th><th>Trades</th><th>WR</th><th>PF</th><th>Market data</th><th>Position</th><th>Health</th><th></th></tr></thead><tbody>
        ...
        {active.map((row) => <article key={row.id} className="instance-worker-row">
          ...
          <section className="instance-worker-column"><Detail label="Timeframe" value={row.timeframe} /><Detail label="Capital / equity" value={`${money(row.capital_allocation)} / ${money(row.execution?.current_equity)}`} /><Detail label="Risk per trade" value={pct(row.risk_per_trade_pct * 100)} /><Detail label="Sizing / entry" value={`${titleCase(row.sizing_mode ?? row.execution?.position_sizing_mode)} / ${titleCase(row.entry_mode ?? row.execution?.entry_mode)}`} /><Detail label="Fill model" value={row.fill_model ?? row.execution?.fill_model ?? "Not available"} /><Detail label="Execution / data" value={`${titleCase(row.execution_mode)} / ${titleCase(row.market_data_mode)}`} /></section>
